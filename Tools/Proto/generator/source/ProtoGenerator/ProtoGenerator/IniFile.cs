...
namespace IniTool
{
	...
	public class IniFile
	{
		...
		{
			return this.Section(section).Get(key);
		}

		public List<string> ReadValues(string section, string key, string prefix = "", int start = 1, int count = 10)
		{
			List<string> results = new List<string>();
			for (int i = 0; i < count; i++)
			{
				string item = ReadValue(section, key + i);
				if (!string.IsNullOrEmpty(item))
					results.Add(prefix + item);
			}
			return results;
		}
